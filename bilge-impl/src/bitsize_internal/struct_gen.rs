...
///
/// Each field starts at its absolute bit offset so holes stay 0.
pub(crate) fn generate_constructor_part(ty: &Type, name: &Ident, shifted_name: &Ident, offset: &TokenStream) -> TokenStream {
    let value_shifted = generate_setter_inner(ty);
