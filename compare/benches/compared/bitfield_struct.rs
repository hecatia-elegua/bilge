...
pub fn combined(input: Input) -> (u32, u32, u64, bool, u16, u16, CommonLpiAffinity) {
    ...
    (
        lpi.control.into_bits(),
        lpi.implementer_identification.into_bits(),
        lpi.redistributor_type.into_bits(),
        clear,
        jep106,
        processor_number,
        affinity,
...
pub fn check(input: Input) {
    ...
    assert_eq!(lpi.control.into_bits(), input.0);
    assert_eq!(lpi.implementer_identification.into_bits(), input.1);
    assert_eq!(lpi.redistributor_type.into_bits(), input.2);
